...
| 14/06/2026 | GET /genres: ~245ms; POST /auth/login: ~280ms | Medido no staging (https://projeto-librum.onrender.com) com a aplicacao ja acordada. GET /genres: media de 5 coletas (277, 258, 211, 270, 207 ms). POST /auth/login: regime de ~280ms (primeira chamada 1485ms por aquecimento de BCrypt/JIT). A primeira requisicao apos hibernacao do plano gratuito do Render (cold start acima de 120s) foi descartada; so a aplicacao acordada conta. Ambos abaixo do RNF02 (2000ms). |